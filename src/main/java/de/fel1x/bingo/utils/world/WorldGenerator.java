    ...
    public WorldGenerator() {
        ...
        for (int i = spawnCuboid.getLowerX(); i < spawnCuboid.getUpperX(); i++) {
            for (int j = spawnCuboid.getLowerZ(); j < spawnCuboid.getUpperZ(); j++) {
                blocks.add(world.getBlockAt(i, 120, j));
            }
        }

        blocks.remove(world.getBlockAt(0, 120, 0));
        world.getBlockAt(0, 120, 0).setType(Material.QUARTZ_PILLAR);

        for (int i = spawnCuboid.getLowerX(); i < spawnCuboid.getUpperX(); i++) {
            for (int height = 0; height < 5; height++) {
                blocks.add(world.getBlockAt(i, 120 + height, spawnCuboid.getLowerZ()));
            }
        }

        for (int i = spawnCuboid.getLowerX(); i < spawnCuboid.getUpperX(); i++) {
            for (int height = 0; height < 5; height++) {
                blocks.add(world.getBlockAt(i, 120 + height, spawnCuboid.getUpperZ()));
            }
        }

        for (int i = spawnCuboid.getLowerX(); i < spawnCuboid.getUpperX(); i++) {
            for (int height = 0; height < 5; height++) {
                blocks.add(world.getBlockAt(spawnCuboid.getLowerX(), 120 + height, i));
            }
        }

        for (int i = spawnCuboid.getLowerX(); i < spawnCuboid.getUpperX(); i++) {
            for (int height = 0; height < 5; height++) {
                blocks.add(world.getBlockAt(spawnCuboid.getUpperX(), 120 + height, i));
            }
        }
        ...
        Bukkit.getScheduler().scheduleSyncRepeatingTask(Bingo.getInstance(),
                () -> blocks.forEach(block -> {
                    if (random.nextBoolean() && !random.nextBoolean()) {
                        block.setType(glassTypes[random.nextInt(glassTypes.length)]);
