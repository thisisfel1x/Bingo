...
public class SkipItemInventory implements InventoryProvider {
    ...
    public void init(Player player, InventoryContents contents) {
        ...
        for (int i = 0; i < bingo.getItemGenerator().getPossibleItems().size(); i++) {
            int finalI = i;
            contents.add(ClickableItem.of(new ItemStack(bingo.getItemGenerator().getPossibleItems().get(i).getMaterial()),
                    clickEvent -> Objects.requireNonNull(clickEvent.getCurrentItem()).setType(bingo.getItemGenerator().generateNewItem(finalI).getMaterial())));
        }
