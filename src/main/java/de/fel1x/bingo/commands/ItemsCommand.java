...
public class ItemsCommand implements CommandExecutor {
    ...
    public ItemsCommand(Bingo bingo) {
        this.bingo = bingo;
        itemGenerator = bingo.getItemGenerator();
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!(sender instanceof Player)) {
            Bukkit.getConsoleSender().sendMessage(bingo.getPrefix() + "§cDu musst ein Spieler sein um das zu tun");
            return false;
        }
    ...
    public Inventory getBingoInventory(BingoPlayer bingoPlayer) {
        ...
        Inventory inventory = Bukkit.createInventory(null, InventoryType.DISPENSER, bingo.getPrefix() + "§7§lItems");

        for (int i = 0; i < itemGenerator.getPossibleItems().size(); i++) {

            ItemStack item = new ItemBuilder(itemGenerator.getPossibleItems().get(i).getMaterial())
                    .setLore("", "§7Schwierigkeit: §b§l" + itemGenerator.getPossibleItems().get(i).getBingoDifficulty().name(), "").toItemStack();
